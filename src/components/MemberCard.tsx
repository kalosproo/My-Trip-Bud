...
export default function MemberCard({ member, share }: { member: Member; share: number }) {
  ...
  return (
    <div className="w-full rounded-2xl bg-white dark:bg-panel ring-1 ring-black/5 dark:ring-white/10 shadow-soft dark:shadow-soft-dark px-4 py-3 flex items-center gap-3">
      {member.photoURL ? (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={member.photoURL} alt="" className="w-9 h-9 rounded-full shrink-0" />
      ) : (
        <div className="w-9 h-9 rounded-full bg-black/5 dark:bg-white/10 flex items-center justify-center text-xs font-semibold text-ink dark:text-bone shrink-0">
          {member.displayName.slice(0, 1)}
        </div>
      )}
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-ink dark:text-bone truncate">{member.displayName}</p>
